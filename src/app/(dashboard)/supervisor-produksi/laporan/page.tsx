...
export default function LaporanProduksiPage() {
  ...
  async function fetchLaporan() {
    setLoading(true);
    const res = await fetch(`/api/produksi?dari=${periode.dari}&sampai=${periode.sampai}`);
  ...
  const totalOutput = data?.produksi?.filter((p: any) => p.status === "DISETUJUI")
    .reduce((sum: number, p: any) => sum + (p.totalOutputKg || 0), 0) || 0;

  const avgEfisiensi = data?.produksi?.filter((p: any) => p.efisiensi)
    .reduce((sum: number, p: any, _: any, arr: any[]) => sum + p.efisiensi / arr.length, 0) || 0;

  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-[#0f1729]">Laporan Produksi</h1>
        <p className="text-gray-500 text-sm mt-1">Rekap produksi per periode</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100">
          ...
          <h2 className="font-semibold text-[#0f1729] text-sm">Detail Batch Produksi</h2>
        </div>
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="text-center py-12 text-gray-400 text-sm">Tidak ada data produksi</td></tr>
            ) : data?.produksi?.map((p: any) => (
