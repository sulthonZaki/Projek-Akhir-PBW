...
export default function LaporanFGPage() {
  ...
  const totalMasuk = data?.transaksi?.filter((t: any) => t.jenis === "MASUK")
    .reduce((sum: number, t: any) => sum + t.jumlah, 0) || 0;
  const totalKeluar = data?.transaksi?.filter((t: any) => t.jenis === "KELUAR")
    .reduce((sum: number, t: any) => sum + t.jumlah, 0) || 0;

  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-[#0f1729]">Laporan Gudang FG</h1>
        <p className="text-gray-500 text-sm mt-1">Rekap transaksi semen per periode</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem" }}>
        <div className="bg-white rounded-xl border border-green-100 p-4">
          ...
          <p className="text-2xl font-bold text-green-600">{formatNumber(totalMasuk)}</p>
          <p className="text-xs text-gray-400 mt-0.5">sak/ton</p>
        </div>
        <div className="bg-white rounded-xl border border-orange-100 p-4">
          <p className="text-xs text-gray-500 mb-1">Total Keluar</p>
          <p className="text-2xl font-bold text-orange-600">{formatNumber(totalKeluar)}</p>
          <p className="text-xs text-gray-400 mt-0.5">sak/ton</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="flex items-center gap-2 px-5 py-3 border-b border-gray-100">
          ...
          <h2 className="font-semibold text-[#0f1729] text-sm">Detail Transaksi</h2>
        </div>
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Semen</th>
