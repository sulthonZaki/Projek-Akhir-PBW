...
export default function BarangKeluarFGPage() {
  ...
  async function fetchData() {
    ...
    setCustomers(masterData.customers?.filter((c: any) => c.aktif) || []);
    setLoading(false);
  }
  ...
  async function handleSubmit() {
    ...
    const res = await fetch("/api/fg/transaksi", {
      ...
      body: JSON.stringify({ ...form, jenis: "KELUAR" }),
    });
    const data = await res.json();
    setSubmitting(false);
    if (!res.ok) return showToast(data.error || "Gagal menyimpan", "error");
    showToast("Transaksi keluar berhasil dicatat");
    setModal(false);
    setForm({ tanggal: new Date().toISOString().split("T")[0], jenisSemenId: "", jumlah: "", satuan: "SAK", gudangId: "", customerId: "", keterangan: "" });
    fetchData();
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#0f1729]">Barang Keluar FG</h1>
          <p className="text-gray-500 text-sm mt-1">Pengeluaran semen ke customer</p>
        </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="text-center py-12 text-gray-400 text-sm">Belum ada transaksi keluar</td></tr>
            ) : transaksi.map((t: any) => (
              <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                ...
                <td className="px-5 py-3 text-sm text-gray-500">{t.customer?.nama || "-"}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-[#0f1729]">Input Barang Keluar FG</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase mb-1.5">Gudang Asal</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase mb-1.5">Customer</label>
                <select value={form.customerId} onChange={(e) => setForm({ ...form, customerId: e.target.value })}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#E8A020] bg-white">
                  <option value="">Pilih customer (opsional)</option>
                  {customers.map((c: any) => <option key={c.id} value={c.id}>{c.nama}</option>)}
                </select>
              </div>
              <div>
